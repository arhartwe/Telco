(function () {
    window.addEventListener('DOMContentLoaded', function () {
        baguetteBox.run('div.photogrid-component--flexbox-container', { filter: /.+/, ignoreClass: 'no-baguette' }); if (!window.__testimonials_initialized && !window.EDIT_MODE) {
            (function () {
                ...
                function testimonial(el) {
                    ...
                    var currentIndex
                    function init() {
                        currentIndex = startIndex; el.classList.add(pubMode)
                        el.style.width = (100 * itemCount) + 'vw'
                    }
                    function nextIndex() {
                        if (currentIndex + 1 === itemCount) { currentIndex = 0 } else { currentIndex++ }
                        return currentIndex;
                    }
